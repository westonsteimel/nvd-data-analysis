...
for package_name, vulns in safety_db.items():
        ...
        for vuln in vulns:
            ...
            if cve:
                ...
                if len(cve_components) != 3:
                    print(f'CVE {cve} in had more than 3 components.')
                    continue
                ...
                if not os.path.exists(cve_path):
                    print(f'No CVE file found for package {ecosystem}:{name}')
...
for package_metadata in updates:
    ...
    outdir = f'output/pyupio/all/{ecosystem}'

    if severity:
        outdir = f'output/pyupio/{severity}/{ecosystem}'
